    ...
    <div class="lg:col-span-2 glass-card rounded-xl p-6 space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="w-full bg-black/30 border border-border-dark rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-primary">{{ old('description') }}</textarea>
        </div>

        <div class="grid grid-cols-2 gap-3">
            <div>
                <label class="block text-xs text-slate-400 mb-1.5 font-medium">Monthly Price <span class="text-red-400">*</span></label>
                <input type="number" name="price_monthly" value="{{ old('price_monthly', 0) }}" min="0" step="0.01" required
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1.5 font-medium">Yearly Price <span class="text-red-400">*</span></label>
                <input type="number" name="price_yearly" value="{{ old('price_yearly', 0) }}" min="0" step="0.01" required
                       class="w-full bg-black/30 border border-border-dark rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-primary" />
            </div>
        </div>
        ...
        <div class="flex items-center gap-6 pt-1">
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                ...
                <input type="checkbox" name="is_public" value="1"
                       ...
                       class="rounded border-border-dark bg-black/30 text-primary focus:ring-primary" />
                <span class="text-sm text-slate-300">Public (visible to users)</span>
            </label>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            @foreach ($featureKeys as $key)
                <div>
                    ...
                    @if (in_array($key, ['analytics_level']))
                        <select name="features[{{ $key }}]"
                                class="w-full bg-black/30 border border-border-dark rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-primary">
                            <option value="none" @selected(old("features.{$key}") === 'none')>None</option>
                            <option value="basic" @selected(old("features.{$key}") === 'basic')>Basic</option>
                            <option value="advanced" @selected(old("features.{$key}") === 'advanced')>Advanced</option>
                        </select>
                    @elseif (in_array($key, ['links_per_month', 'custom_domains_count', 'team_members_count']))
                        <input type="number" name="features[{{ $key }}]"
                               value="{{ old("features.{$key}", 0) }}" min="0"
                               class="w-full bg-black/30 border border-border-dark rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-primary" />
                    @else
                        {{-- Boolean feature --}}
